...
<?php
$countries = []; $classes = [];
foreach( $people->posts as $team_member ) {
	foreach( wp_get_post_tags($team_member->ID) as $tag ) {
		if( !in_array($tag->name, $countries) ) {
			$countries[] = $tag->name;
			$classes[]   = strtolower($tag->name);
		}
	}
}
...
<?php if( $people->have_posts() ): ?>
	<div class="mixitup-container">
		<?php while( $people->have_posts() ) : $people->the_post(); ?>
			...
			<?php $countries = implode(" ", array_map(function($tag) {return strtolower($tag->name);}, wp_get_post_tags(get_the_ID())) ?: []); ?>
			<?php $country_title = "<span class='small' style='opacity:0.6; position:relative; top:-5px;'>".strtoupper($countries)."</span>"; ?>
			...
			<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 mix team-member responsive-bg <?php echo $countries; ?> <?php echo $ventures; ?> <?php echo $seeds; ?> <?php echo $roles; ?>"
				...
				<article class="team-member-info-overlay content-padding-wrapper show-person-modal"
					data-title="<?php echo htmlspecialchars($person['title']); ?><?php echo strpos($countries, ' ')===false ? '<br>'.htmlspecialchars($country_title) : ''; ?>"
